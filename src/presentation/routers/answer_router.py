...
@router.put(path="/{id}")
async def edit_vote(id: str, update: AnswerSchema, cmd: FromDishka[UpdateAnswerCommand]):
    print(dict(**update.model_dump(exclude_unset=True)))
    return await cmd(id=id, update_obj=dict(**update.model_dump(exclude_unset=True)))


@router.delete("/{id}")
async def delete_vote(id: str, cmd: FromDishka[DeleteAnswerCommand]):
    return await cmd(id)
